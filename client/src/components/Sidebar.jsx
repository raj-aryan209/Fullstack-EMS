...
import { MenuIcon, XIcon, UserIcon, LayoutGridIcon, FileTextIcon, CalendarIcon, DollarSignIcon, SettingsIcon, ChevronRightIcon, LogOut, LogOutIcon } from 'lucide-react'
...
const Sidebar = () => {
  ...
  const sidebarContent = (
    <>
          ...
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                ...
                <Icon size={17} className={`shrink-0 ${isActive ? 'text-indigo-300' : 'text-slate-400 group-hover:text-slate-300'}`} />
                <span className="flex-1 text-sm">{item.name}</span>
                {isActive && <ChevronRightIcon className="w-4 h-4 text-indigo-500/50" />}
              </NavLink>
            )
          })}
        ...
        <button onClick={handlelogout} className='flex items-center gap-3 w-full px-3 py-2.5 rounded-md text-[13px] font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/8 trasnsition-all duration-150'>
  ...
  return (
    <>
      ...
      <aside className='hidden lg:flex flex-col h-full w-65 bg-linear-to-b from-slate-900 via-slate-900 to-slate-950 text-white shrink-0 border-r border-white/4'>
       ...
       {/* Sidebar- mobile */}
       <aside className={`lg:hidden fixed inset-y-0 left-0 w-72 bg-linear-to-b from-slate-900 via-slate-900 text-white z-50 flex flex-col transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full"}`}>
          {sidebarContent}
       </aside>
