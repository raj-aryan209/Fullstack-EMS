...
export const inngest = new Inngest({ id: "Raj-EMS" });
...
const attendamceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron" },
  {cron : "TZ=Asia/Kolkata 0 0 6 * * *"}, // 06:00 UTC = 11:30 AM IST
    async ({ step }) => {
    ...
    // step 3: Get employee ID's on approved leave today
      ...
      // Step 4: Get employee ID's who already checked in today 
      const checkedInIds = await step.run("get-checked-in-ids", async()=>{
        const attendance = await Attendance.find({
          date: { $age: new Date (today.startUTC), $1t: new Date(today.endUTC) }, 

        }).lean();
        return attendace.map((a)=>a.employeeId.toString())
      })
      // Step 5: Filter absent employees (not on leave and not checked in)
      const absentEmployees = activeEmployess.filter((emp)=>!onLeaveIds.includes(emp._id) && !checkedInIds.includes(emp._id))
